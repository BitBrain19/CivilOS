...
import { getMBByProject, measurementBookEntries, MBEntry } from '@/data/measurementBook';
import {
  BookOpen,
  Printer,
  Download,
...
export default function MeasurementBookPage() {
  ...
  const [selectedMB, setSelectedMB] = useState<MBEntry>(mbList[0] || measurementBookEntries[0]);
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-xl border border-border shadow-sm-warm">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base font-semibold text-ink">Digital Measurement Book (MB)</h2>
            <span className="badge badge-accent font-document italic">Official Record</span>
          </div>
          <p className="text-xs text-muted mt-0.5">
            Standard civil engineering measurement ledger (L &times; B &times; H) linked directly to certified site DPR records
          </p>
        </div>
      ...
      {printSuccess && (
        <div className="flex items-center gap-2 text-success text-xs bg-success/10 border border-success/30 rounded-lg p-3 count-up">
          ...
          <span>Generating tamper-evident digital copy with cryptographic signature certificate.</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs font-semibold text-accent">{mb.mbNo}</span>
                    <span
                  ...
                  <div className="text-xs font-medium text-ink mt-2 line-clamp-2">{mb.boqItemDesc}</div>
        ...
        <div className="lg:col-span-8 bg-white border border-border-strong rounded-xl shadow-lg-warm p-8 md:p-10 font-document text-ink">
          ...
          <div className="text-center border-b-2 border-ink pb-6 mb-6">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-ink uppercase">Digital Measurement Book</h1>
            <div className="text-sm font-sans text-muted mt-1">
              Contractor: <span className="font-semibold text-ink">{activeProject.contractor}</span>
            </div>
            <div className="text-xs font-sans text-muted mt-0.5">
              Project: <span className="font-semibold text-ink">{activeProject.name}</span> ({activeProject.code})
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-surface/50 p-4 rounded-lg border border-border mb-6 font-sans text-xs">
            <div>
              ...
              <div className="font-mono font-bold text-accent mt-0.5">{selectedMB.mbNo}</div>
            </div>
            <div>
              <div className="text-muted text-2xs uppercase">Measurement Date</div>
              <div className="font-semibold text-ink mt-0.5">{selectedMB.date}</div>
            </div>
            <div>
              <div className="text-muted text-2xs uppercase">Site Engineer</div>
              <div className="font-semibold text-ink mt-0.5 truncate">{selectedMB.checkedBy.split('(')[0]}</div>
            </div>
            <div>
              <div className="text-muted text-2xs uppercase">Approval Status</div>
              <div className="font-semibold text-success mt-0.5 uppercase tracking-wide">{selectedMB.status}</div>
            </div>
          </div>

          {/* Item Description */}
          <div className="mb-6 p-3.5 bg-surface/30 rounded border border-border/80">
            <div className="text-2xs font-sans uppercase font-bold text-muted tracking-wider">BOQ Line Item</div>
            <div className="text-base font-semibold text-ink mt-0.5">{selectedMB.boqItemDesc}</div>
            <div className="text-xs font-sans text-muted mt-1 italic">Unit of Measurement: {selectedMB.unit}</div>
          </div>
            ...
            <table className="w-full border-collapse document-table">
              <thead>
                <tr className="bg-surface/80 border-y border-border-strong text-2xs">
                  ...
                  <th className="text-right w-28">Quantity ({selectedMB.unit})</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {selectedMB.subItems.map((item) => (
                  <tr key={item.slNo} className="hover:bg-surface/30">
                    <td className="text-center font-mono text-xs text-muted">{item.slNo}</td>
                    <td className="text-xs">{item.description}</td>
                    <td className="text-right font-mono text-xs text-muted">{item.noNos}</td>
              ...
              <tfoot>
                <tr className="border-t-2 border-ink bg-surface/60 font-sans">
                  <td colSpan={6} className="text-right font-bold text-xs uppercase pr-3 py-2.5">
                ...
                <tr className="border-t border-border font-sans text-2xs text-muted">
                  <td colSpan={6} className="text-right pr-3 py-1.5">Previous Cumulative Certified Quantity:</td>
                  <td className="text-right font-mono py-1.5">{selectedMB.cumulativePrevQty.toFixed(2)} {selectedMB.unit}</td>
                </tr>
                <tr className="border-t border-border-strong font-sans text-xs font-bold text-ink bg-surface/80">
                  <td colSpan={6} className="text-right pr-3 py-2 uppercase">Total Cumulative Quantity to Date:</td>
                  <td className="text-right font-mono text-success py-2">{selectedMB.cumulativeThisMB.toFixed(2)} {selectedMB.unit}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-border font-sans text-xs">
            <div className="space-y-2">
              ...
              <div className="bg-surface/40 p-2.5 rounded border border-border space-y-1 text-2xs text-muted">
                <div>DPR Ref: <span className="font-mono text-ink">{selectedMB.dprRef.join(', ')}</span></div>
            ...
            <div className="space-y-2">
              <div className="font-semibold text-ink">Engineer Certification Remarks</div>
          ...
          <div className="mt-8 pt-6 border-t-2 border-ink grid grid-cols-2 gap-8 font-sans">
            <div>
              ...
              <div className="text-2xs uppercase font-bold text-ink mt-1">Measured & Recorded By</div>
            ...
            <div className="text-right">
              ...
              <div className="text-2xs uppercase font-bold text-ink mt-1">Checked & Approved By</div>
              <div className="text-2xs text-muted">
                {selectedMB.approvedBy || 'Er. Prashant Koirala (Project Manager)'}
              </div>
            </div>
          </div>
        </div>
