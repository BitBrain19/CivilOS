...
export default function RABillPage() {
  ...
  const [bills, setBills] = useState<RABill[]>(projectBills.length > 0 ? projectBills : raBills);
  const [selectedBill, setSelectedBill] = useState<RABill>(bills[0] || raBills[0]);
  const [toastMsg, setToastMsg] = useState('');

  const formatNPR = (val: number) => `₨ ${val.toLocaleString('en-IN', { maximumFractionDigits: 2 })}`;
  ...
  const handleUpdateStatus = (newStatus: 'submitted' | 'approved' | 'paid') => {
    ...
    setToastMsg(`Bill #${selectedBill.billNo} status updated to "${newStatus.toUpperCase()}".`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-xl border border-border shadow-sm-warm">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base font-semibold text-ink">Running Account (RA) Billing</h2>
            <span className="badge badge-accent font-document italic">Interim Payment Certificate</span>
          </div>
          <p className="text-xs text-muted mt-0.5">
            Claim generation directly computed from verified Digital MB records with standard statutory deductions
          </p>
        </div>
        <div className="flex items-center gap-2.5">
          ...
          <button
            onClick={() => {
              setToastMsg(`Bill #${selectedBill.billNo} exported as signed PDF statement.`);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-4 space-y-3">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs font-bold text-accent">{bill.billNo}</span>
                    <span
                  ...
                  <div className="flex items-baseline justify-between mt-2">
                    <div className="text-sm font-bold text-ink">{formatNPR(bill.netPayable)}</div>
                  ...
                  <div className="flex items-center justify-between text-2xs text-muted mt-3 pt-2.5 border-t border-border/60">
                    ...
                    <span className="font-mono text-2xs text-muted">{bill.mbRefs.join(', ')}</span>
                  </div>
                </button>
              );
            })}
          ...
          <div className="bg-white p-4 rounded-xl border border-border shadow-sm-warm space-y-3">
            <div className="text-xs font-semibold text-ink">Bill Approval Workflow</div>
            <div className="text-2xs text-muted leading-relaxed">
              Current stage: <span className="font-bold text-ink uppercase">{selectedBill.status}</span>
            </div>
        ...
        <div className="lg:col-span-8 bg-white border border-border-strong rounded-xl shadow-lg-warm p-8 md:p-10 font-document text-ink">
          ...
          <div className="border-b-2 border-ink pb-6 mb-6">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
              <div>
                ...
                <h1 className="text-2xl font-bold tracking-tight text-ink uppercase">Running Account Bill</h1>
                <div className="text-sm font-sans text-muted mt-1">
                  Bill No: <span className="font-mono font-bold text-accent">{selectedBill.billNo}</span>
                </div>
              </div>
              <div className="text-right sm:text-right font-sans text-xs">
                ...
                <div className="text-2xs text-muted mt-1.5">Date: {selectedBill.billDate}</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-surface/50 p-4 rounded-lg border border-border mb-6 font-sans text-xs">
            <div>
              <div className="text-muted text-2xs uppercase font-bold">Employer / Client</div>
              <div className="font-semibold text-ink mt-0.5">{selectedBill.client}</div>
            </div>
            <div>
              <div className="text-muted text-2xs uppercase font-bold">Contractor</div>
              <div className="font-semibold text-ink mt-0.5">{selectedBill.contractor}</div>
            </div>
            <div>
              <div className="text-muted text-2xs uppercase font-bold">MB Cross-References</div>
              <div className="font-mono font-semibold text-accent mt-0.5">{selectedBill.mbRefs.join(', ')}</div>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-border">
                {selectedBill.lineItems.map((item) => (
                  <tr key={item.itemNo} className="hover:bg-surface/30">
                    <td className="font-mono text-xs text-muted">{item.itemNo}</td>
                    <td className="text-xs font-medium">{item.description}</td>
                    <td className="text-2xs text-muted">{item.unit}</td>
                    <td className="text-right font-mono text-xs text-muted">{item.prevCertifiedQty.toFixed(2)}</td>
                    <td className="text-right font-mono text-xs font-semibold text-ink">{item.thisBillQty.toFixed(2)}</td>
                    <td className="text-right font-mono text-xs text-muted">{item.totalQty.toFixed(2)}</td>
                    <td className="text-right font-mono text-xs text-muted">₨ {item.rate.toLocaleString()}</td>
          ...
          <div className="border-t-2 border-ink pt-4 mb-6 font-sans">
            <div className="max-w-xs ml-auto space-y-2 text-xs">
              <div className="flex justify-between py-1 border-b border-border text-muted">
                ...
                <span className="font-bold text-ink font-mono">{formatNPR(selectedBill.grossAmount)}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-border text-muted">
              ...
              <div className="flex justify-between py-2 border-y-2 border-ink text-sm font-bold bg-surface/60 px-2 rounded">
                <span className="uppercase text-ink">Net Payable to Contractor:</span>
                <span className="font-mono text-accent">{formatNPR(selectedBill.netPayable)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-surface/30 p-3 rounded border border-border font-sans text-xs mb-8">
            <div className="text-2xs uppercase font-bold text-muted tracking-wider">Bill Notes</div>
            <p className="text-2xs text-muted italic mt-0.5">&ldquo;{selectedBill.remarks}&rdquo;</p>
          </div>
          ...
          <div className="pt-4 border-t-2 border-ink grid grid-cols-2 md:grid-cols-3 gap-6 font-sans">
            <div>
              ...
              <div className="text-2xs uppercase font-bold text-ink mt-1">Prepared By (Contractor)</div>
            ...
            <div>
              ...
              <div className="text-2xs uppercase font-bold text-ink mt-1">Verified By (Site Eng.)</div>
            ...
            <div className="text-right md:text-left">
              ...
              <div className="text-2xs uppercase font-bold text-ink mt-1">Certified By (Client)</div>
              <div className="text-2xs text-muted">Project Manager / Resident Eng.</div>
            </div>
          </div>
        </div>
      </div>
