...
import { Smartphone, MapPin, Camera, Clock, CheckCircle2, Plus } from 'lucide-react';
      ...
      <div className="flex-1 min-w-0">
        <div className="bg-white rounded-xl border border-border shadow-sm-warm overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            <div>
              <h2 className="text-sm font-semibold text-ink">Today&apos;s Site Log</h2>
              <p className="text-xs text-muted mt-0.5">Tuesday, 1 October 2024</p>
            </div>
            <span className="badge badge-accent">{todayEntries.length} entries</span>
          </div>
          ...
          {todayEntries.length > 0 ? (
              ...
              {todayEntries.map((entry) => (
                <div key={entry.id} className="px-5 py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium text-ink">{entry.boqItemDesc}</div>
                      <div className="text-xs text-muted mt-0.5">{entry.gang}</div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-sm font-semibold text-ink">{entry.quantityToday} <span className="text-muted font-normal text-xs">{entry.unit}</span></div>
                      {entry.certified ? (
                        <span className="badge badge-success text-2xs mt-1">Certified</span>
                      ) : (
                        <span className="badge badge-muted text-2xs mt-1">Pending</span>
                      )}
                    </div>
                  </div>
                  <div className="mt-2.5 flex items-center gap-3 text-2xs text-muted">
                    <span className="flex items-center gap-1"><MapPin size={10} /> {entry.geotag.split(',')[0]}</span>
                    <span className="flex items-center gap-1"><Clock size={10} /> {new Date(entry.timestamp).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  {entry.remarks && (
                    <div className="mt-2 text-xs text-muted bg-surface rounded px-2.5 py-1.5 italic">{entry.remarks}</div>
                  )}
                  {/* Photo placeholder */}
                  <div className="mt-3 flex gap-2">
                    ...
                    <div className="w-16 h-12 rounded-md bg-gradient-to-br from-accent/20 to-accent/10 border border-accent/20 flex items-center justify-center">
                      <span className="text-2xs text-accent font-medium">Site Photo</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="py-12 text-center text-muted text-sm">No entries yet today</div>
          )}
        </div>
        ...
        {prevEntries.length > 0 && (
          <div className="mt-4 bg-white rounded-xl border border-border shadow-sm-warm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-border">
              <h3 className="text-xs font-semibold text-muted uppercase tracking-wider">30 September 2024 — Certified</h3>
            </div>
            <div className="divide-y divide-border">
              {prevEntries.map((entry) => (
                <div key={entry.id} className="px-5 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-xs font-medium text-ink">{entry.boqItemDesc}</div>
                      <div className="text-2xs text-muted">{entry.gang}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs font-semibold text-ink">{entry.quantityToday} {entry.unit}</div>
                    {entry.mbRef && <div className="text-2xs text-accent mt-0.5">{entry.mbRef}</div>}
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex-shrink-0 flex self-start justify-center lg:justify-start">
        <div className="w-full max-w-[340px] bg-white rounded-2xl overflow-hidden shadow-lg-warm">
          ...
          <div className="bg-accent px-4 py-3 flex items-center gap-2">
            ...
            <div>
              <div className="text-white text-xs font-medium">Daily Progress Entry</div>
              <div className="text-white/60 text-2xs">Shantinagar Residency</div>
            </div>
          </div>
          ...
          <div className="bg-base p-4 space-y-3">
            ...
            <div>
              <label className="text-2xs font-medium text-muted uppercase tracking-wider block mb-1">BOQ Activity</label>
              <select
                value={selectedBOQ}
              ...
              >
                {boqItems.map((b) => (
                  <option key={b.id} value={b.id}>{b.itemNo} — {b.description}</option>
                ))}
              </select>
            </div>
            ...
            {selectedItem && (
              <div className="bg-accent/8 border border-accent/15 rounded-lg px-3 py-2 flex justify-between text-2xs">
                ...
                <span className="font-semibold text-accent">
                  {(selectedItem.quantity - selectedItem.completedQty).toFixed(1)} {selectedItem.unit}
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-2xs font-medium text-muted uppercase tracking-wider block mb-1">Qty Today</label>
                <div className="flex items-center border border-border rounded-lg bg-white overflow-hidden">
                  <input
                    ...
                    placeholder="0.00"
                  />
                  <span className="px-2 text-2xs text-muted border-l border-border bg-surface">{selectedItem?.unit}</span>
                </div>
              </div>
              <div>
                <label className="text-2xs font-medium text-muted uppercase tracking-wider block mb-1">Gang / Naike</label>
                <select
                  value={gang}
            ...
            <div>
              <label className="text-2xs font-medium text-muted uppercase tracking-wider block mb-1">Remarks</label>
              <textarea
                value={remarks}
            ...
            <div className="flex items-center gap-2 bg-surface rounded-lg px-3 py-2 border border-border">
              ...
              <div>
                ...
                <div className="text-2xs text-muted">Shantinagar, Kathmandu · GPS confirmed</div>
              </div>
            </div>
            ...
            <div className="border-2 border-dashed border-border rounded-lg p-3 flex flex-col items-center gap-1.5">
              ...
              <span className="text-2xs text-muted">Tap to attach site photo</span>
