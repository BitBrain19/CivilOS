    ...
    <aside className="fixed left-0 top-0 h-screen w-60 bg-ink flex flex-col z-40 select-none">
      ...
      <div className="px-5 pt-5 pb-4 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-white font-semibold text-sm leading-tight tracking-tight">CivilOS</div>
            <div className="text-white/40 text-2xs leading-tight">Narayan Constructions</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-b border-white/10">
        <div className="text-white/30 text-2xs font-medium uppercase tracking-widest mb-1.5 px-1">Active Project</div>
        <button
          onClick={() => setDropdownOpen(!dropdownOpen)}
          className="w-full flex items-center gap-2 px-2 py-2 rounded-md hover:bg-white/8 transition-colors group"
        >
          <span className="text-sm">{projectTypeIcon[activeProject.type]}</span>
          <div className="flex-1 text-left min-w-0">
            <div className="text-white/90 text-xs font-medium leading-tight truncate">{activeProject.name}</div>
            <div className="text-white/35 text-2xs leading-tight">{activeProject.code}</div>
          </div>
          <ChevronDown
            size={12}
            ...
            {allProjects.map((p) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <div className={`text-xs leading-tight truncate ${p.id === activeProject.id ? 'text-white' : 'text-white/70'}`}>
                    {p.name}
                  </div>
      ...
      <nav className="flex-1 px-3 py-3 overflow-y-auto">
        <div className="text-white/25 text-2xs font-medium uppercase tracking-widest mb-2 px-2">Modules</div>
          ...
          {navItems.map(({ href, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                ...
                <span className="font-medium leading-tight text-xs">{label}</span>
              </Link>
            );
          })}
        </div>
      ...
      <div className="px-5 py-4 border-t border-white/10">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="text-white/80 text-xs font-medium">Pushkar Jha</div>
